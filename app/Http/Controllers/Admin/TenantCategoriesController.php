...
use App\DataGrids\Admin\TenantCategoryDataGrid;

class TenantCategoriesController extends Controller
{
    public function index(Tenant $tenant): View|JsonResponse
    {
        ...
        try {
            if (request()->ajax()) {
                return datagrid(TenantCategoryDataGrid::class)->process();
            }

            return view('admin.tenants.categories.index', [
                'tenant' => $tenant,
            ]);
        } finally {
